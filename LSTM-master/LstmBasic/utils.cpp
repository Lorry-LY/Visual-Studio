...
int ReadConfig(const string &config_file, Config* config)
{
	...
	if (fin.is_open())
	{
		...
		config->batchSize = atoi(dict["batchSize"].c_str());	//批量大小
		config->epochNum = atoi(dict["epochNum"].c_str());		//历元观测值
		config->useBatchAdapt = atoi(dict["useBatchAdapt"].c_str());	//使用批量适应
		config->len_in = atoi(dict["len_in"].c_str());		//输入长度
		config->len_out = atoi(dict["len_out"].c_str());		//输出长度
		config->weightRange = atof(dict["weightRange"].c_str());		//权重范围
		config->learningRate = atof(dict["learningRate"].c_str());		//训练速率
...
void GradientCheck(Net *net, MatrixXd *Ys, MatrixXd &w, MatrixXd &this_d_w)
{
	...
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
		{
			w(i, j) += eps;
			(*net).FF();
			MatrixXd e1 = (*Ys) - (*net).mh;
			double loss1 = (e1.array()*e1.array()).sum() / 2 / (*Ys).rows();
			w(i, j) -= 2 * eps;
			(*net).FF();
			MatrixXd e2 = (*Ys) - (*net).mh;
			double loss2 = (e2.array()*e2.array()).sum() / 2 / (*Ys).rows();
...
{
	cout << "ix: ";
	GradientCheck(&net, net.Ys, net.wix, net.this_d_wix);
	cout << "ih: ";
	GradientCheck(&net, net.Ys, net.wih, net.this_d_wih);
	cout << "ic: ";
	GradientCheck(&net, net.Ys, net.wic, net.this_d_wic);
	cout << "fx: ";
	GradientCheck(&net, net.Ys, net.wfx, net.this_d_wfx);
	cout << "fh: ";
	GradientCheck(&net, net.Ys, net.wfh, net.this_d_wfh);
	cout << "fc: ";
	GradientCheck(&net, net.Ys, net.wfc, net.this_d_wfc);
	cout << "cx: ";
	GradientCheck(&net, net.Ys, net.wcx, net.this_d_wcx);
	cout << "ch: ";
	GradientCheck(&net, net.Ys, net.wch, net.this_d_wch);
	cout << "ox: ";
	GradientCheck(&net, net.Ys, net.wox, net.this_d_wox);
	cout << "oh: ";
	GradientCheck(&net, net.Ys, net.woh, net.this_d_woh);
	cout << "oc: ";
	GradientCheck(&net, net.Ys, net.woc, net.this_d_woc);
	cout << "x: ";
	GradientCheck(&net, net.Ys, *(net.Xs), net.d_x);
	...
	for (int ei = 0; ei < config.epochNum; ei++)
	{
		...
		for (int bi = 0; bi < batchNum; bi++)
		{
			...
			for (int i = startIdx; i < endIdx; i++)
			{
				...
				net.Ys = &(trainXYs[i][1]);
				net.FF();
				MatrixXd e = trainXYs[i][1] - net.mh;
				loss += (e.array()*e.array()).sum() / 2 / trainXYs[i][0].rows();
				net.BPTT(e);
	...
	for (int i = 0; i < test_size; i++)
	{
		...
		for (int mi = 0; mi < net.Xs->rows(); mi++)//words
		{
			...
			for (int ti = 0; ti < net.Ys->cols(); ti++)//types
			{
				if (net.mh(mi, ti)>max)
				{
					maxTypeIdx = ti; max = net.mh(mi, ti);
				}
			}
