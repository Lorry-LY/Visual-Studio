...
int		consumeNumber[MAX_BUFFER_NUM];
...
void function(void* p, DWORD& wait_for_semaphore, int& m_serial, int m_thread_request[], int& i);
...
int main(void)
{
	...
	empty_semaphore = CreateSemaphore(NULL, n_Buffer_or_Critical, n_Buffer_or_Critical,
		LPCUWSTR("semaphore_for_empty"));
	h_mutex = CreateMutex(NULL, FALSE, LPCUWSTR("mutex_for_update"));
	...
	for (int j = 0; j < (int)n_Thread; j++) {
		...
		h_Semaphore[j + 1] = CreateSemaphore(NULL, 0, n_Thread, LPCUWSTR((lp.c_str())));
	}
...
void Consume(void* p)
{
	...
	for (int i = 0; i < m_requestNum; i++)
	{
		...
		consumeNumber[m_thread_request[i]]++;
	}
	...
	while (true)
	{
		...
		for (int i = 0; i < m_requestNum; i++) {
			...
			//请求消费下一个产品
			for (int j = 0; j < n_Buffer_or_Critical; j++)
			{
				if (m_thread_request[i] == Buffer_Critical[j])
				{
					function(p, wait_for_semaphore, m_serial, m_thread_request, i);
					m_thread_request[i] = -2;
				}
				else if(Buffer_Critical[j]!=-1)
				{
					isnothing++;
				}
			}
		...
		for (int i = 0; i < m_requestNum; i++)
		{
			if (m_thread_request[i] == -2)number++;
		}
		if (number == m_requestNum)break;
		Sleep(1000);
	}
...
void function(void* p, DWORD& wait_for_semaphore, int& m_serial, int m_thread_request[], int& i)
{
	...
	consumeNumber[m_thread_request[i]]--;
	if (!consumeNumber[m_thread_request[i]]) {
